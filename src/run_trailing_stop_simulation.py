...
capital = 100.0
stop_pct = 0.15
trailing_pct = 0.20  # 20% drop from peak
    ...
    for ticker, gap, op, pc in selections:
        ...
        if df_1m is not None and len(df_1m) > 0:
            ...
            current_time = pd.to_datetime(current_time)
                ...
                # Current trailing stop level based on peak BEFORE this minute
                current_stop = max(initial_stop, high_max * (1 - trailing_pct))
            ...
            peak_runup = (high_max - entry_price) / entry_price
        else:
            ...
            try:
                ...
                initial_stop = entry_price * (1 - stop_pct)
                trailing_stop = high_val * (1 - trailing_pct)
                
                # If low hits the initial stop, assume worst case (SL hit)
                if low_val <= initial_stop:
                    exit_price = initial_stop
                    exit_type = "SL"
                    exit_time = "Intraday"
                # If low hits trailing stop, exit at trailing stop
                    ...
                    exit_time = "16:00"
                ret = (exit_price - entry_price) / entry_price
                peak_runup = (high_val - entry_price) / entry_price
        ...
        print(f"  - {ticker}: Gap {gap*100:+.2f}%, Peak {peak_runup*100:+.2f}%, Return {ret*100:+.2f}% ({exit_type} @ {exit_time}, Source: {source})")
...
md += f"* **청산 규칙**: 고점 대비 **-20% 하락 시 트레일링 스탑(TS) 청산** / 기본 손절 제한 **-15%** / 장마감(EOD) 청산\n"
md += f"  * *동적 스탑 가격*: `max(시초가 * 0.85, 당일 최고가 * 0.80)`\n"
md += f"  * *보수적 백테스트*: 1분봉 데이터가 없을 경우 일봉 데이터로 대체하며, 당일 저가가 초기 손절선에 닿으면 **손절(-15%) 처리**하고, 초기 손절선은 안 닿았으나 최고점 대비 -20% 수준 이하로 내려갔다면 **트레일링 스탑 청산**으로 처리하는 보수적 방식을 적용했습니다.\n\n"
...
for t in trade_log:
    ...
    md += "| 종목명 | 갭상승률 | 당일 최대 상승률 (Peak) | 결과 유형 | 수익률 | 데이터 소스 | 비고 |\n"
    md += "| :---: | :---: | :---: | :---: | :---: | :---: | :--- |\n"
    for p in t['Positions']:
        md += f"| {p['ticker']} | {p['gap']*100:+.2f}% | {p['peak']*100:+.2f}% | {p['type']} | {p['ret']*100:+.2f}% | {p['source']} | {p['exit_time']} 청산 |\n"
    md += "\n"

output_md_path = str(Path(__file__).resolve().parent.parent / "results" / "trailing_stop_results.md")
