...
# Run the simulation for the best strategy
capital = 100.0
pt = 0.40
sl = 0.20
...
for date_str in sorted_days:
    selections = daily_selections[date_str]
    if len(selections) < 2:
        print(f"[{date_str}] Skipped - less than 2 selections.")
        continue
    ...
    if exit_type == "SL":
        ...
        loss_dates.append(date_str)
    else:
        consecutive_losses = 0
        
    chronological_log.append({
        "Date": date_str,
        "Ticker": ticker,
        "Gap": gap,
        "Type": exit_type,
    ...
    print(f"[{date_str}] {ticker}: Return {ret*100:+.2f}% ({exit_type}). Capital: {old_capital:.2f} -> {capital:.2f}")

print("\n=== CONSECUTIVE LOSS ANALYSIS ===")
print(f"Max Consecutive Stop Losses (-20%): {max_consecutive_losses}")
print(f"Total Stop Losses hit: {len(loss_dates)} / {len(chronological_log)}")

# Save to JSON
with open("drawdown_analysis.json", "w") as f:
    json.dump({
        "max_consecutive_losses": max_consecutive_losses,
        "total_trades": len(chronological_log),
        "total_sl": len(loss_dates),
        "trades": chronological_log
    }, f, indent=4)
