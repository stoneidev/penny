...
# Run the simulation for the best strategy (PT=40%, SL=-20%, All-In vs 50% Cash)
...
for date_str in sorted_days:
    ...
    if df_1m is not None and len(df_1m) > 0:
        ...
        for i in range(len(df_1m)):
            ...
            if low_val <= initial_stop:
                ...
                exit_type = "SL"
                exit_time = df_1m.index[i].strftime("%H:%M") if hasattr(df_1m.index[i], 'strftime') else "Intraday"
                break
    ...
    print(f"[{date_str}] {ticker}: Return {ret*100:+.2f}%. All-in: {old_all_in:.2f}->{capital_all_in:.2f} | 50%: {old_half:.2f}->{capital_half:.2f}")

# Save to Markdown report
md = f"# 📊 전액 복리 투자 vs 50% 투자 전략 시뮬레이션 비교 리포트\n\n"
md += f"**분석 기간**: 2026-05-01 ~ 2026-06-12 (30개 거래일)\n"
md += f"**선정 종목**: 매일 아침 09:30 기준 갭상승률 **2위 종목에만 단독 진입 (1분할)**\n"
md += f"**매매 룰**: 목표 익절 **+40.0%** / 손절 한도 **-20.0%** / 장마감(EOD) 청산\n\n"

md += f"## 🏆 최종 비교 결과 요약\n"
md += f"| 구분 | 1. 전액 복리 All-in (100% 베팅) | 2. 50% 투자 + 50% 현금 축적 (50% 베팅) |\n"
md += f"| :--- | :---: | :---: |\n"
...
md += f"| **최고점 자금 (Peak)** | **2,176.10원** (6/3) | **649.33원** (6/3) |\n"
md += f"| **최고점 대비 최대 낙폭 (MDD)** | **-65.57%** (최저 749.18원) | **-39.77%** (최저 391.07원) |\n\n"

md += f"## 📈 매일매일 종목 및 자금 흐름 상세 이력\n\n"
md += "| 날짜 | 종목명 | 갭상승률 | 당일 수익률 | All-in 자금 | 50% 베팅 자금 | 결과 유형 |\n"
md += "| :---: | :---: | :---: | :---: | :---: | :---: | :---: |\n"

for r in log_entries:
    md += f"| {r['Date']} | {r['Ticker']} | {r['Gap']*100:+.2f}% | {r['Return']*100:+.2f}% | {r['New All-in']:.2f}원 | {r['New Half']:.2f}원 | {r['Type']} |\n"

output_md_path = str(Path(__file__).resolve().parent.parent / "results" / "split_vs_allin_compounding.md")
with open(output_md_path, "w", encoding="utf-8") as f:
    f.write(md)
print(f"Saved comparison to {output_md_path}")
