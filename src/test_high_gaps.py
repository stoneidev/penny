"""극단적인 갭상승 종목(갭 >= +200%) 대상 시뮬레이션:
매일 아침 09:30 시초가 기준 갭상승률 전체 1위 종목 중 갭이 +200% 이상인 종목만 거래.
...
def main():
    ...
    # 1. 갭 200% 이상인 종목 스크리닝
    high_gaps = picks[picks['p1_gap'] >= 2.00].copy()
    print(f"Number of days with Rank 1 Gap >= 200%: {len(high_gaps)}")

    # 2. 캐시 데이터 로드
    print("Downloading intraday data for extreme gap-up tickers...")
    data_cache = {}
